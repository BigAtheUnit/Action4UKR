document.addEventListener("DOMContentLoaded", function () {
  ...
  // --- Share Helper ---
  // Each platform gets a unique, natural message to avoid spam detection
  function shareOn(platform) {
    var pageUrl = window.location.href;
    var url = encodeURIComponent(pageUrl);
    var shareUrls = {
      x: "https://twitter.com/intent/tweet?url=" + url + "&text=" + encodeURIComponent("I just emailed my MP about supporting Ukraine. It only takes a minute \u2014 you can do it too:"),
      telegram: "https://t.me/share/url?url=" + url + "&text=" + encodeURIComponent("Use this free tool to email your MP about Ukraine. Took me less than a minute."),
      facebook: "https://www.facebook.com/sharer/sharer.php?u=" + url,
      whatsapp: "https://api.whatsapp.com/send?text=" + encodeURIComponent("Have you written to your MP about Ukraine yet? This tool makes it really easy \u2014 " + pageUrl)
    };
    if (shareUrls[platform]) window.open(shareUrls[platform], '_blank', 'noopener,noreferrer');
  }

  // Header share buttons
  document.getElementById('xShare').addEventListener('click', () => shareOn('x'));
  document.getElementById('telegramShare').addEventListener('click', () => shareOn('telegram'));
  document.getElementById('facebookShare').addEventListener('click', () => shareOn('facebook'));
  document.getElementById('whatsappShare').addEventListener('click', () => shareOn('whatsapp'));

  // Footer share buttons
  const footerX = document.getElementById('footerXShare');
  const footerTelegram = document.getElementById('footerTelegramShare');
  const footerFacebook = document.getElementById('footerFacebookShare');
  const footerWhatsApp = document.getElementById('footerWhatsAppShare');
  if (footerX) footerX.addEventListener('click', () => shareOn('x'));
  if (footerTelegram) footerTelegram.addEventListener('click', () => shareOn('telegram'));
  if (footerFacebook) footerFacebook.addEventListener('click', () => shareOn('facebook'));
  if (footerWhatsApp) footerWhatsApp.addEventListener('click', () => shareOn('whatsapp'));

  // Success section share buttons
  const successX = document.getElementById('successXShare');
  const successWhatsApp = document.getElementById('successWhatsAppShare');
  if (successX) successX.addEventListener('click', () => shareOn('x'));
  if (successWhatsApp) successWhatsApp.addEventListener('click', () => shareOn('whatsapp'));
  ...
  // --- Start Button / Form Reveal ---
  startButton.addEventListener("click", function () {
    formSection.classList.remove("hidden");
    formSection.classList.add("form-entering");
    ...
    formSection.addEventListener("animationend", function () {
      formSection.classList.remove("form-entering");
    }, { once: true });
  });

  // --- Populate MP Dropdown ---
  populateMPs(mpDataByCountry["UK"]);

  function populateMPs(mpData) {
    mpSelect.innerHTML = '<option value="" disabled selected>Search for your MP or constituency...</option>';
  ...
  function updatePreview() {
    ...
    if (selectedMP && selectedMP.dataset.name && selectedMP.dataset.constituency) {
      mpName = escapeHTML(selectedMP.dataset.name);
      constituency = escapeHTML(selectedMP.dataset.constituency);
    }

    const userName = escapeHTML(userNameInput.value || "[Your Name]");
    const emailPreview = '<p>Dear ' + mpName + ',</p>' +
  ...
  document.getElementById("emailForm").addEventListener("submit", function (e) {
    e.preventDefault();
    const selectedMP = mpDataByCountry["UK"]?.find(function (mp) {
      return mp.email === mpSelect.value;
    });

    if (!selectedMP) return;

    const subject = "Urgent Support for Ukraine's Full Right to Defend Itself";
    const body = "Dear " + selectedMP.name + ",\n\n" +
    ...
    // Show fallback and success sections
  ...
  function openEmailClient(baseURL, mpEmail) {
    if (!mpEmail) {
      showToast("Please select an MP first.", "error");
      return;
    }
    ...
    window.open(fullLink, '_blank', 'noopener');
  }
});
